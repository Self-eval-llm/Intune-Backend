...
CREATE TABLE IF NOT EXISTS modelComp_50k (
    ...
    -- Student output (generated incrementally)
    student_output TEXT,                -- Generated student output
    generation_latency FLOAT,           -- Time to generate (seconds)
    worker_id INT,                      -- Which distributed worker generated this
    
    -- Checkpoint tracking
    checkpoint INT,                     -- Which 5K checkpoint (1-10)
...
-- SELECT checkpoint, COUNT(*) as pending
-- FROM modelComp_50k
-- WHERE student_output IS NULL
-- GROUP BY checkpoint;
